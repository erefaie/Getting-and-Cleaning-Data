...
## add new column to both datasets for activity 
...
tidy_ds <- tidy_ds[, colnames(tidy_ds[, -(grep("meanFreq", colnames(tidy_ds)))])]

## update the column of activity 
tidy_ds$Activity <- activity_label[tidy_ds$Activity, 2]

## create dataset for average
tidy_ds_g <- aggregate(tidy_ds[, c(1:66)], by=list(tidy_ds$Activity), FUN=mean, na.rm=TRUE)
